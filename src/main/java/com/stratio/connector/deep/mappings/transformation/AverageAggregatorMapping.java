...
public class AverageAggregatorMapping implements Function<Cells, Cells> {
    ...
    public Cells call(Cells row) throws Exception {
        Cell sumCell =
                (Cell) row.getCellByName(averageColumnInfo.getTable(), averageSumField);
        Cell countCell =
                (Cell) row.getCellByName(averageColumnInfo.getTable(), averageCountField);
        ...
        resultCell = Cell.create(averageColumnInfo.getColumnName(), average);
